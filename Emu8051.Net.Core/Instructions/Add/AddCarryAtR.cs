...
{
    [BitVariant(1)]
    public class AddCarryAtR : AddCarryBase
    {
        private const byte _baseOpCode = 0x36;
        ...
        public AddCarryAtR(int variantNumber)
        {
        ...
        {
            var targetRAMAddress = memory.GetBankRegisterValue(_variantNumber);
            return $"{Mnemonic} A [0x{memory.Accumulator:X2}], @R{_variantNumber} [0x{targetRAMAddress:X2}; 0x{memory.RAM[targetRAMAddress]:X2}]";
        }
    }
}